...
type SavedCreation = {
  ...
  photoCropX: number; photoCropY: number; photoFeather: number; photoBottomCrop?: number; nameArc: number;
};
...
export default function FuncCardWorkspace() {
  ...
  const [photoBottomCrop, setPhotoBottomCrop] = useState(0);
  const [photoEditorOpen, setPhotoEditorOpen] = useState(false);
  const [pendingPhotoUrl, setPendingPhotoUrl] = useState("");
  const [pendingPhotoZoom, setPendingPhotoZoom] = useState(1);
  const [pendingPhotoCropX, setPendingPhotoCropX] = useState(0);
  const [pendingPhotoCropY, setPendingPhotoCropY] = useState(0);
  const [pendingPhotoBottomCrop, setPendingPhotoBottomCrop] = useState(0);
  ...
  const safePhotoCropX = clamp(photoCropX, -50, 50);
  const safePhotoCropY = clamp(photoCropY, -50, 50);
  const safePhotoFeather = clamp(photoFeather, 8, 55);
  const safePhotoBottomCrop = clamp(photoBottomCrop, 0, 45);
  ...
  const openPhotoEditor = (url = photoUrl) => { if (!url) return; setPendingPhotoUrl(url); setPendingPhotoZoom(safePhotoZoom); setPendingPhotoCropX(safePhotoCropX); setPendingPhotoCropY(safePhotoCropY); setPendingPhotoBottomCrop(safePhotoBottomCrop); setPhotoEditorOpen(true); };
  const uploadPhoto = (file?: File) => { if (!file) return; if (!file.type.startsWith("image/")) return setMessage("Please choose an image file."); const reader = new FileReader(); reader.onload = () => { const url = String(reader.result); setPendingPhotoUrl(url); setPendingPhotoZoom(1); setPendingPhotoCropX(0); setPendingPhotoCropY(0); setPendingPhotoBottomCrop(0); setPhotoEditorOpen(true); setMessage("Align the face in the guide, then place the crop on the card."); }; reader.onerror = () => setMessage("The portrait could not be loaded. Try another image."); reader.readAsDataURL(file); };
  const applyPendingPhoto = () => { setPhotoUrl(pendingPhotoUrl); setPhotoZoom(pendingPhotoZoom); setPhotoCropX(pendingPhotoCropX); setPhotoCropY(pendingPhotoCropY); setPhotoBottomCrop(pendingPhotoBottomCrop); setPhotoEditorOpen(false); setSelectedLayer("photo"); setMessage("Portrait placed. You can reopen the crop editor or move the complete portrait layer on the card."); };
  ...
  return <section className="func-workspace">
    <header className="func-intro"><div><p className="eyebrow">FUTBOL UNC NOVELTY CARD</p><h2>FUNC Card Studio</h2></div><p>Build your own Futbol Unc Novelty Card, align your portrait, and tune every detail before downloading it.</p></header>
    <div className="func-layout">
      ...
      <form className="func-controls" onSubmit={(event) => event.preventDefault()}>
        ...
        <CollapsiblePanel title="Upload & crop" eyebrow="PORTRAIT" meta={photoUrl ? "Photo loaded" : "No photo"} className="func-editor-panel" initiallyOpen={false}><label className="func-upload">Choose player photo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => uploadPhoto(event.target.files?.[0])} /></label>{photoUrl ? <button type="button" className="func-edit-photo" onClick={() => openPhotoEditor()}>Reopen face alignment</button> : null}<label>Face crop / zoom <span>{safePhotoZoom.toFixed(2)}×</span><input type="range" min="1" max="2.8" step="0.01" value={safePhotoZoom} onChange={(event) => setPhotoZoom(clamp(Number(event.target.value), 1, 2.8))} /></label><label>Edge feather <span>{safePhotoFeather}%</span><input type="range" min="8" max="55" step="1" value={safePhotoFeather} onChange={(event) => setPhotoFeather(Number(event.target.value))} /></label><label>Flat bottom crop <span>{safePhotoBottomCrop}%</span><input type="range" min="0" max="45" step="1" value={safePhotoBottomCrop} onChange={(event) => setPhotoBottomCrop(Number(event.target.value))} /><small>Raises the portrait's straight bottom edge so it stays above the name ribbon.</small></label><div className="func-control-pair"><label>Face left / right<input type="range" min="-50" max="50" step="0.5" value={safePhotoCropX} onChange={(event) => setPhotoCropX(Number(event.target.value))} /></label><label>Face up / down<input type="range" min="-50" max="50" step="0.5" value={safePhotoCropY} onChange={(event) => setPhotoCropY(Number(event.target.value))} /></label></div><button type="button" className="secondary" onClick={() => { setPhotoZoom(1); setPhotoCropX(0); setPhotoCropY(0); setPhotoBottomCrop(0); setPhotoFeather(masterDefaults[templateKey]?.photoFeather || 32); }}>Reset portrait</button></CollapsiblePanel>
    ...
    {photoEditorOpen ? <div className="func-photo-editor-backdrop" role="presentation"><section className="func-photo-editor" role="dialog" aria-modal="true" aria-labelledby="func-photo-editor-title"><div className="func-photo-editor-heading"><div><p className="eyebrow">ALIGN BEFORE PLACING</p><h3 id="func-photo-editor-title">Find and frame the face</h3></div><button type="button" aria-label="Close photo editor" onClick={() => setPhotoEditorOpen(false)}>×</button></div><p>Move the focal point until the face sits inside the square and the eyes meet the center cross. The bottom crop previews the flat edge above the name ribbon.</p><div className="func-photo-crop-stage"><img src={pendingPhotoUrl} alt="Portrait crop preview" style={{ objectPosition: `${50 + pendingPhotoCropX}% ${50 + pendingPhotoCropY}%`, transform: `scale(${pendingPhotoZoom})`, clipPath: `inset(0 0 ${pendingPhotoBottomCrop}% 0)` }} /><div className="func-face-guide" aria-hidden="true"><span>+</span></div></div><div className="func-photo-editor-controls"><label>Scale <span>{pendingPhotoZoom.toFixed(2)}×</span><input type="range" min="1" max="2.8" step="0.01" value={pendingPhotoZoom} onChange={(event) => setPendingPhotoZoom(Number(event.target.value))} /></label><label>Move left / right<input type="range" min="-50" max="50" step="0.5" value={pendingPhotoCropX} onChange={(event) => setPendingPhotoCropX(Number(event.target.value))} /></label><label>Move up / down<input type="range" min="-50" max="50" step="0.5" value={pendingPhotoCropY} onChange={(event) => setPendingPhotoCropY(Number(event.target.value))} /></label><label>Flat bottom crop <span>{pendingPhotoBottomCrop}%</span><input type="range" min="0" max="45" step="1" value={pendingPhotoBottomCrop} onChange={(event) => setPendingPhotoBottomCrop(Number(event.target.value))} /></label></div><div className="func-photo-editor-actions"><button type="button" onClick={() => setPhotoEditorOpen(false)}>Cancel</button><button type="button" className="primary" onClick={applyPendingPhoto}>Place photo on card</button></div></section></div> : null}
  </section>;
}

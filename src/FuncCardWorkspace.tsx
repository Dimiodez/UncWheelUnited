  ...
  return <section className="func-workspace">
    ...
    <div className="func-layout">
      ...
      <form className="func-controls" onSubmit={(event) => event.preventDefault()}>
        ...
        <section><p className="eyebrow">PORTRAIT</p><h3>Upload & crop</h3><label className="func-upload">Choose player photo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => uploadPhoto(event.target.files?.[0])} /></label><label>Face crop / zoom <span>{safePhotoZoom.toFixed(2)}×</span><input type="range" min="1" max="2.8" step="0.01" value={safePhotoZoom} onChange={(event) => { const nextZoom = clamp(Number(event.target.value), 1, 2.8); const nextPan = (nextZoom - 1) * 50; setPhotoZoom(nextZoom); setPhotoCropX((current) => clamp(current, -nextPan, nextPan)); setPhotoCropY((current) => clamp(current, -nextPan, nextPan)); }} /><small>The minimum preserves the original soft oval without exposing photo corners.</small></label><label>Edge feather <span>{safePhotoFeather}%</span><input type="range" min="8" max="55" step="1" value={safePhotoFeather} onChange={(event) => setPhotoFeather(Number(event.target.value))} /></label><div className="func-control-pair"><label>Face left / right<input type="range" min={-maxPhotoPan} max={maxPhotoPan || .01} step="0.1" value={safePhotoCropX} disabled={maxPhotoPan < .1} onChange={(event) => setPhotoCropX(clamp(Number(event.target.value), -maxPhotoPan, maxPhotoPan))} /></label><label>Face up / down<input type="range" min={-maxPhotoPan} max={maxPhotoPan || .01} step="0.1" value={safePhotoCropY} disabled={maxPhotoPan < .1} onChange={(event) => setPhotoCropY(clamp(Number(event.target.value), -maxPhotoPan, maxPhotoPan))} /></label></div><button type="button" className="secondary" onClick={() => { setPhotoZoom(1); setPhotoCropX(0); setPhotoCropY(0); setPhotoFeather(32); }}>Reset portrait</button></section>

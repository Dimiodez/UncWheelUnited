  ...
  return <section className="func-workspace">
    ...
    <div className="func-layout">
      ...
      <form className="func-controls" onSubmit={(event) => event.preventDefault()}>
        <CollapsiblePanel title="Choose a card" eyebrow="BORDER COLLECTION" meta={template.name} className="func-editor-panel" initiallyOpen><label>Card border<select value={templateKey} onChange={(event) => { const key = event.target.value as TemplateKey; setTemplateKey(key); setMessage(`${TEMPLATES[key].name} border selected. Its layout can be tuned independently.`); }}><optgroup label="Available UFL borders">{(Object.entries(TEMPLATES) as [TemplateKey, typeof TEMPLATES[TemplateKey]][]).map(([key, option]) => <option value={key} key={key}>{option.name}</option>)}</optgroup><optgroup label="6v6 team themes — files coming soon">{TEAMS.map(([key, team]) => <option value={`team-${key}`} disabled key={key}>{team.replace(/^UFL /, "")} theme — upload pending</option>)}</optgroup></select></label><div className="func-border-preview"><img src={frameUrl} alt="" /><div><strong>{template.name}</strong><span>Available now</span></div></div></CollapsiblePanel>

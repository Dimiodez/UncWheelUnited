...
export default function FuncCardWorkspace() {
  ...
  const exportCard = async () => {
    try {
      ...
      const nameSpot = point("name"); const cardName = name.trim().slice(0, 22).toUpperCase() || "UNC PLAYER"; context.font = `900 ${Math.round(canvas.width * .047 * nameSpot.scale)}px Arial Black, Impact, sans-serif`; const maxNameWidth = canvas.width * .58 * nameSpot.scale; const naturalNameWidth = context.measureText(cardName).width; const nameWidthScale = Math.min(1, maxNameWidth / naturalNameWidth); const charWidths = [...cardName].map((character) => context.measureText(character).width); const totalNameWidth = charWidths.reduce((sum, width) => sum + width, 0) * nameWidthScale; let nameX = nameSpot.x - totalNameWidth / 2; [...cardName].forEach((character, index) => { const progress = cardName.length === 1 ? 0 : index / (cardName.length - 1) * 2 - 1; const offsetY = -nameArc * canvas.width * .0015 * (1 - progress * progress); const rotation = nameArc * progress * .0025; const characterWidth = charWidths[index] * nameWidthScale; context.save(); context.translate(nameX + characterWidth / 2, nameSpot.y + offsetY); context.rotate(rotation); context.scale(nameWidthScale, 1); context.fillText(character, 0, 0); context.restore(); nameX += characterWidth; }); context.fillStyle = currentColors.stats;
    ...
    <div className="func-layout">
      <div className="func-card-column">
        <div className="func-card func-card-editable" ref={cardRef} style={{ aspectRatio: `${template.width}/${template.height}`, "--func-name-ink": currentColors.name, "--func-stat-ink": currentColors.stats, "--func-top-ink": currentColors.top } as React.CSSProperties} onPointerDown={() => setSelectedLayer("photo")} aria-label="Editable FUNC player card preview">
          ...
          <div className="func-layer func-player-name" {...layerProps("name")} aria-label={name.trim() || "UNC PLAYER"}>{[...(name.trim() || "UNC PLAYER")].map((character, index, characters) => { const progress = characters.length === 1 ? 0 : index / (characters.length - 1) * 2 - 1; return <span key={`${character}-${index}`} style={{ transform: `translateY(${-nameArc * .7 * (1 - progress * progress)}px) rotate(${nameArc * progress * .15}deg)` }}>{character === " " ? "\u00a0" : character}</span>; })}</div>
      ...
      <form className="func-controls" onSubmit={(event) => event.preventDefault()}>
        ...
        <section><p className="eyebrow">PORTRAIT</p><h3>Upload & crop</h3><label className="func-upload">Choose player photo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => uploadPhoto(event.target.files?.[0])} /></label><label>Face crop / zoom <span>{photoZoom.toFixed(2)}×</span><input type="range" min="0.45" max="2.8" step="0.01" value={photoZoom} onChange={(event) => setPhotoZoom(Number(event.target.value))} /></label><label>Edge feather <span>{photoFeather}%</span><input type="range" min="0" max="70" step="1" value={photoFeather} onChange={(event) => setPhotoFeather(Number(event.target.value))} /></label><div className="func-control-pair"><label>Face left / right<input type="range" min="-100" max="100" value={photoCropX} onChange={(event) => setPhotoCropX(Number(event.target.value))} /></label><label>Face up / down<input type="range" min="-100" max="100" value={photoCropY} onChange={(event) => setPhotoCropY(Number(event.target.value))} /></label></div><button type="button" className="secondary" onClick={() => { setPhotoZoom(1); setPhotoCropX(0); setPhotoCropY(0); setPhotoFeather(32); }}>Reset portrait</button></section>

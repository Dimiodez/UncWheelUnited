...
export default function LeaguePhasePanel({ names, onCreateKnockout }: Props) {
  ...
  return <section className="league-phase-board">
    ...
    {fixtures.length > 0 && <div className="league-phase-layout"><div className="league-table-card"><h3>League standings</h3><div className="league-table"><div className="table-head"><span>#</span><span>Player</span><span>P</span><span>W</span><span>D</span><span>L</span><span>GD</span><span>Pts</span></div>{table.map((row, index) => <div className={index < directPlaces ? "direct" : index < directPlaces + playoffPlaces ? "playoff" : ""} key={row.name}><span>{index + 1}</span><strong>{row.name}</strong><span>{row.played}</span><span>{row.won}</span><span>{row.drawn}</span><span>{row.lost}</span><span>{row.goalDifference > 0 ? `+${row.goalDifference}` : row.goalDifference}</span><b>{row.points}</b></div>)}</div><div className="qualification-key"><span>Direct to knockout</span><span>Qualification playoff</span></div></div>
